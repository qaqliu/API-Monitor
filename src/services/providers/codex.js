...
function resolveAuthPath(apiKey) {
  return (apiKey && apiKey.trim()) ? apiKey.trim() : getDefaultAuthPath();
}
...
async function fetchUsage(apiKey) {
  const authPath = resolveAuthPath(apiKey);
  ...
  const accountId = authData.tokens && authData.tokens.account_id;
...
async function fetchBalance(apiKey) {
  return mapCodexUsage(await fetchUsage(apiKey), 'codex');
}

function createProvider(options) {
  const id = options.id || PROVIDER_ID;
  return {
    id,
    name: options.name || PROVIDER_NAME,
    logo: LOGO,
    fetchBalance: async (apiKey) => mapCodexUsage(await fetchUsage(apiKey), id),
  };
}

function mapCodexUsage(data, providerId) {
  const rateLimit = data.rate_limit || {};
  const primary = rateLimit.primary_window || {};
  const weekly = selectWeeklyWindow(rateLimit);
  ...
  return {
    ...
    primary_limit_window_seconds: primary.limit_window_seconds ?? FIVE_HOUR_SECONDS,
    weekly_used_percent: weekly.used_percent ?? null,
    weekly_reset_at: weekly.reset_at ?? null,
    weekly_reset_after_seconds: weekly.reset_after_seconds ?? null,
    weekly_limit_window_seconds: weekly.limit_window_seconds ?? WEEK_SECONDS,
...
function selectWeeklyWindow(rateLimit) {
  const windows = [
  ...
  const weeklyByDuration = windows.find(window =>
    Number(window.limit_window_seconds) >= WEEK_SECONDS - 60
  );

  return weeklyByDuration || rateLimit.secondary_window || rateLimit.primary_window || {};
}
...
module.exports = createProvider({ id: PROVIDER_ID, name: PROVIDER_NAME });
module.exports.createProvider = createProvider;
module.exports.mapCodexUsage = mapCodexUsage;
